# Historical and historical-nat temperature at baseball stadiums
...
# ssp models
ssp_exp = "ssp245"
ssp_models = np.array([x for x in sorted(os.listdir(loc_cmip6+ssp_exp+"/tasmax_day/")) if (x.endswith(".nc"))])
ssp_models_prefix = np.array([x.split("_")[2]+"_"+x.split("_")[4] for x in ssp_models])

# intersection of all three
#models = reduce(np.intersect1d,(hist_models_prefix,histnat_models_prefix,ssp245_models_prefix))
...
existing_data = np.array([x.split("_")[0]+"_"+x.split("_")[1] for x in os.listdir(loc_out)])
...
for m in models:
    ...
    model_ssp = mname+"_"+ssp_exp+"_"+mreal
    ...
    tx_ds_ssp = xr.open_mfdataset(loc_cmip6+ssp_exp+"/tasmax_day/"+"tasmax_day"+"_"+model_ssp+"*.nc",
                                concat_dim="time")
    ...
    # concat
    tx_hist_final = xr.concat([tx_hist_noleap,tx_ssp_noleap],dim="time")
    ...
    lat_histn = tx_ds_histn.coords["lat"].load()
    tm_histn_ind = (tm_histn.dt.year>=y1)&(tm_histn.dt.year<=y2)
    lat_histn_ind = (lat_histn>=latmin)&(lat_histn<=latmax)
    tx_histnat = tx_ds_histn.tasmax[tm_histn_ind,lat_histn_ind,:].load()
    if tx_histnat.max()>200:
        tx_histnat = tx_histnat-273.15
    tx_histnat_final = xr_strip_leapdays(tx_histnat)

    # regrid
    if np.amin(tx_hist_final.coords["lon"].values)<0:
        tx_hist_final.coords["lon"] = tx_hist_final.coords["lon"].values % 360
    if np.amin(tx_histnat_final.coords["lon"].values<0):
        tx_histnat_final.coords["lon"] = tx_histnat_final.coords["lon"].values % 360
    tx_hist_regrid = tx_hist_final.interp(lat=lat,lon=lon)
    tx_histnat_regrid = tx_histnat_final.interp(lat=lat,lon=lon)
    del([tx_hist_final,tx_histnat_final])

    # establish calendar
    tm = xr.cftime_range(start=str(y1)+"-01-01",end=str(y2)+"-12-31",
                            freq="D",calendar="noleap")
    tx_hist_regrid.coords["time"] = tm
    tx_histnat_regrid.coords["time"] = tm

    # create data -- park x time
    print("calculating park-level data",flush=True)
    park_tx_hist = xr.DataArray(np.full((len(parks),len(tm)),np.nan),
                                coords=[parks,tm],dims=["park","time"])
    park_tx_histnat = xr.DataArray(np.full((len(parks),len(tm)),np.nan),
                                coords=[parks,tm],dims=["park","time"])
    ...
    for p in np.arange(0,len(parks),1):
        ...
        tx_hist_park = tx_hist_regrid[:,(lat_ind-1):(lat_ind+2),(lon_ind-1):(lon_ind+2)].mean(dim=["lat","lon"])
        tx_histnat_park = tx_histnat_regrid[:,(lat_ind-1):(lat_ind+2),(lon_ind-1):(lon_ind+2)].mean(dim=["lat","lon"])
        park_tx_hist[p,:] = tx_hist_park
        park_tx_histnat[p,:] = tx_histnat_park

    # combine into ds and write out
    park_ds = xr.Dataset({"tx_hist":(["park","time"],park_tx_hist),
                          "tx_histnat":(["park","time"],park_tx_histnat)},
    ...
    park_ds.attrs["variable_description"] = "Daily tx from CMIP6 historical and historical-nat"
    park_ds.attrs["created_from"] = os.getcwd()+"/CMIP6_Historical_ParkTemp.py"
    park_ds.attrs["spatial_averaging"] = "9 grid cells closest to park location"

    fname_out = loc_out+m+"_historical_natural_baseball_park_tx_"+str(y1)+"-"+str(y2)+".nc"
